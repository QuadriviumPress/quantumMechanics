...
Spin states taught us that a ket is abstract and a column is its representation
in a chosen basis. Position uses the same logic with a continuously labeled
basis. The generalized ket $|x\rangle$ represents an ideal position outcome,
and the position-space wavefunction is
...
The Dirac delta is not an ordinary finite function. It is defined by its action
under an integral:
...
Ideal position kets are therefore not normalizable physical states. Real
preparations are wave packets with finite width.

It is worth seeing why an ordinary finite function cannot play the role of
$\langle x|x'\rangle$. Suppose instead that $\langle x|x'\rangle=g(x-x')$ for
some bounded function $g$. Completeness would then require

```{math}
\langle x'|\psi\rangle=\int g(x'-x)\psi(x)\,dx
```

to equal $\psi(x')$ for *every* state $|\psi\rangle$. A bounded kernel
convolves and smooths a function; it cannot reproduce the function's exact
value at a point unless it is concentrated entirely at $x=x'$ with unit
weight, and no ordinary function does that at a single point while
integrating to one. Only the singular limit of an infinitely narrow, infinitely
tall spike — the Dirac delta — has the required sifting property. This is the
same logic that made a discrete basis's Kronecker delta $\delta_{mn}$ work for
spin: orthonormality is exactly what is needed to read off a coefficient by
taking an inner product. The continuum version of that same requirement forces
delta-function normalization instead of unit inner products, and forces the
generalized kets $|x\rangle$ to sit just outside the space of normalizable
states.

:::{note} A generalized basis is a bookkeeping device
No apparatus prepares $|x\rangle$ itself: it would require infinite momentum
uncertainty and infinite energy. What is physically prepared are normalizable
wave packets, expressed as superpositions $\int\psi(x)|x\rangle\,dx$ over the
generalized basis. The basis kets earn their keep by making the expansion
coefficients — the wavefunction — easy to extract via
{eq}`position-wavefunction`, exactly as $|+z\rangle,|-z\rangle$ did for spin.
:::
...
Unlike a discrete probability, $|\psi(x)|^2$ is a probability **density** and
has units of inverse length. The probability of one exact point in a continuous
distribution is zero, even where the density is nonzero.

This is a genuine conceptual shift from the spin chapters, not merely a change
of notation. There, $|\langle+z|\psi\rangle|^2$ was already a probability. Here
one more step — multiplying by an interval $dx$ or integrating over a finite
range — is required before a number between $0$ and $1$ appears. Forgetting
this step is the most common bookkeeping error when moving from discrete to
continuous quantum mechanics: a "large" value of $|\psi(x)|^2$ at some point
says only that a narrow interval there carries appreciable probability, not
that the point itself is likely.
...
Boundary behavior matters. Integration by parts shows that $\hat p$ is
Hermitian only when boundary terms vanish or appropriate boundary conditions
relate them. An operator is not defined by a differential expression alone;
its allowed domain is part of its definition. Chapter 7 will meet this issue
...
domain, while a differential expression alone would not guarantee it.

### Example 6.2: phase carries mean momentum

Let $\psi(x)=f(x)e^{ik_0x}$, where $f$ is real, normalized, and vanishes at
infinity. Then
...
because $\int f f'\,dx=0$. The density $|\psi|^2=f^2$ does not reveal $k_0$;
the spatial phase does. As in the spin interferometer, phase information
becomes observable only through a suitable measurement.

## 6.4 Momentum states and Fourier transforms

Momentum eigenstates satisfy $\hat p|p\rangle=p|p\rangle$. In position space,
...
These are basis changes, not dynamical evolution. Normalization is preserved:
$\int|\psi(x)|^2dx=\int|\phi(p)|^2dp=1$.

A broad packet in position generally requires a narrow range of wave numbers;
a narrow position packet requires many. This Fourier fact underlies the
position–momentum uncertainty relation. It is a property of pairs of Fourier
partners in general, with no quantum content of its own; what quantum
mechanics adds is the physical claim that $\phi(p)$, not merely $\psi(x)$,
is a genuine probability amplitude for an equally real observable. A classical
wave packet's Fourier width is a statement about its mathematical shape; a
quantum wavefunction's Fourier width is simultaneously a statement about the
distribution of a different physical quantity's measured values.
...
{eq}`xp-commutator`. The uncertainty is not caused by a badly calibrated
position detector kicking the particle. It describes the spreads in two
different ensembles prepared in the same state.
...
for some real number $\lambda$. It is worth solving this condition explicitly
for $\hat A=\hat x$, $\hat B=\hat p$ to see why the minimizing states are
Gaussian and not some other shape. In the position representation, with
$\langle p\rangle$ momentarily set to zero for simplicity,
...
Normalizability requires the exponent to decay, so $\lambda$ must be negative;
writing $\lambda=-2\sigma_x^2/\hbar$ reproduces exactly the Gaussian profile of
equation {eq}`gaussian-packet` (restoring $\langle p\rangle=\hbar k_0$ reinserts
the phase $e^{ik_0x}$, which does not affect $\Delta x$ or $\Delta p$). No
other line shape solves equation {eq}`minimum-uncertainty-condition`: any
state that is not Gaussian must have $\Delta x\,\Delta p>\hbar/2$ strictly.
This is why Gaussian wave packets, rather than being an arbitrary convenient
example, are *the* states of least joint position–momentum uncertainty.

### Example 6.3: overlap of two wave packets

How distinguishable are two Gaussian packets prepared with the same width
$\sigma_x$ and mean wave number $k_0$ but centers $x_1$ and $x_2$? Their
overlap is
...
$(x-x_1)^2+(x-x_2)^2=2(x-\bar x)^2+\tfrac12(x_1-x_2)^2$ separates the exponent
into a piece that integrates to the normalization and a piece that depends
only on the separation:
...
separated by less than a width overlap substantially, and no measurement can
sort individual particles between them with certainty. This is the continuous
counterpart of the Stern–Gerlach statement that only exactly orthogonal spin
states are perfectly distinguishable — here "exactly orthogonal" is replaced
by "infinitely far apart," and finite separation always leaves some residual
overlap.
...
Momentum is therefore the generator of spatial translations, just as the
Hamiltonian is the generator of time translations and spin components
generate rotations. The pattern is always the same: a Hermitian observable
exponentiates into a unitary operator that implements a continuous symmetry,
and the observable's expectation value is conserved exactly when that symmetry
leaves the Hamiltonian unchanged. Chapter 9 will show that orbital and spin
angular momentum generate rotations in precisely this sense, so that the
rotation operators already used for spin in Chapter 4 are a special case of a
single unifying structure that also produces {eq}`translation-operator` here.
...
An ideal sharp position measurement uses projectors $|x\rangle\langle x|$ only
as a mathematical limit. A detector pixel covering a region $R$ corresponds
to
...
outcome state, with finite detector resolution replacing an ideal one-
...
Its magnitude is unchanged, so the momentum distribution is constant. The
phase is nonlinear in $p$, however, and transforming back to position causes a
packet to spread. For the minimum-uncertainty Gaussian of equation
{eq}`gaussian-packet`,
...
The center moves classically:

```{math}
\langle x\rangle_t=x_0+\frac{\hbar k_0}{m}t.
```

A narrow initial packet spreads faster because it contains a broader range of
momenta and therefore a broader range of velocities.

### Group and phase velocity

A plane-wave component has phase velocity
$v_{\mathrm{ph}}=\omega/k=\hbar k/(2m)$, while the packet envelope travels at
...
meaningless — it governs how the fine ripples inside the envelope move — but
it is not what a particle detector downstream registers as "how fast the
particle arrived." Only the envelope carries probability, and only the
envelope's speed should be compared with a classical velocity.
...
These resemble Hamilton's equations, but in general
$\langle V'(\hat x)\rangle\ne V'(\langle x\rangle)$. The center of a narrow
packet in a slowly varying potential approximately follows a classical
trajectory. For quadratic potentials the equations close exactly for the
means. The reason is purely algebraic: the discrepancy
$\langle V'(\hat x)\rangle-V'(\langle x\rangle)$ is controlled by the higher
moments of $\hat x-\langle x\rangle$ through a Taylor expansion of $V'$ about
$\langle x\rangle$, and every term beyond the first involves at least
$\langle(\hat x-\langle x\rangle)^2\rangle$ or a higher power. If $V'$ is
linear in $x$ — equivalently, if $V$ is at most quadratic — that Taylor
expansion has no higher terms at all, and the mean-value equations close
exactly regardless of how wide the packet is. For any other potential, the gap
is generally nonzero and grows with the packet's spread.

Ehrenfest's theorem does not turn a quantum state into a classical point. The
packet retains uncertainties, may spread, and may split into reflected and
transmitted pieces. Classical behavior emerges only for suitable states,
observables, potentials, and time scales.
...
The mean follows the classical constant-acceleration trajectory exactly, even
though the position distribution need not remain narrow.
...
separate question: for the free particle it grows without bound
...
$\phi(p)$. One has changed the basis used to describe the same ket. A physical
position measurement, by contrast, changes conditional predictions through
state update. This distinction is the continuous analogue of rewriting a spin
ket in the $x$ basis versus sending it through an $x$ analyzer.

:::{important} Representation change versus measurement
Computing $\phi(p)=\langle p|\psi\rangle$ from a known $\psi(x)$ is arithmetic:
it uses one fixed state and asks what its coefficients look like in a
different, already-chosen basis. Performing a momentum measurement is
physical: it interacts the system with an apparatus, produces one random
outcome, and updates the state. The two operations can produce numerically
identical-looking distributions $|\phi(p)|^2$ — one as a computed property of
the original state, one as a set of relative frequencies from repeated trials
— but only the second involves selecting an outcome and preparing a new state
...
:::{dropdown} Answer
No. The phase cancels from all expectation values. A position-dependent phase,
such as $e^{ik_0x}$, can change momentum statistics.
:::
...
:::{dropdown} Answer
No. Its magnitude is constant over all space and it cannot be normalized in the
ordinary sense. It is a generalized basis state; physical beams are modeled by
normalizable superpositions with a range of momenta.
:::
...
overlap of $\exp(-36/8)=\exp(-4.5)\approx0.011$, so the two states are nearly
orthogonal and a position measurement almost always lands cleanly in the
region associated with the correct packet.
:::
...
multiplies $\phi(p)$ by the pure phase $e^{-iap/\hbar}$ and leaves $|\phi(p)|^2$
unchanged. Sliding a packet in space redistributes nothing among its momentum
components; only *where* it is found changes, not *how fast* it is found to be
moving.
:::

### Concept check 6.6

For the harmonic oscillator, does Ehrenfest's theorem guarantee that a widely
spread-out state behaves like a classical particle in every respect?
